...
const actions = {
  async fetchStatuses({ commit, rootState }, projectId) {
    ...
    try {
      ...
      const response = await api.get(`/tasks/statuses/?project_id=${projectId}`, {
  ...
  async createStatus({ commit, rootState }, { name, order, projectId, color }) {
    ...
    try {
      ...
      const response = await api.post('/tasks/statuses/', payload, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  async updateStatus({ commit, rootState }, { statusId, name, order }) {
    ...
    try {
      ...
      const response = await api.patch(`/tasks/statuses/${statusId}/`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  async deleteStatus({ commit, rootState }, statusId) {
    ...
    try {
      const token = rootState.auth.token
      await api.delete(`/tasks/statuses/${statusId}/`, {
        headers: { Authorization: `Bearer ${token}` },
      })
